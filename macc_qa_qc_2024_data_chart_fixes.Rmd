...
library(patchwork)
library(gridExtra) # https://stackoverflow.com/questions/1249548/side-by-side-plots-with-ggplot2

# for Agriculture sector MACC, set ymin = 0. Price values for mitigation below zero are so low that the options above zero can't be seen on the plot
...
MACC_01242025 <- read_csv("data/MACC_01242025.csv")
MACC_df <- MACC_01242025

# add in residual line
# add a bar for 2019 data

paste("Emissions Sources:", paste(unique(MACC_df$source), collapse = ","))
...
unique(MACC_df$sector)
# "AGRICULTURE" "ENERGY"      "WASTE"   
sectors <- unique(MACC_df$sector)

for (i in 1:length(sectors)) {
  macc_byTech(MACC_df, sectorSelect = sectors[i])
...
macc_byTech_ag_upper = function(df, 
                       sectorSelect=NULL, 
                       sourceSelect=NULL, 
                       yearSelect=2030, 
                       priceMax=100) {
...
#      mutate(ymax = 0)
#    title = "Mitigation At or Below $0"
  }
...
macc_byTech_ag_upper(MACC_df_ag, sectorSelect = "AGRICULTURE",  yearSelect = 2030)
ggsave("output/AGRICULTURE_2030_v2024_upper.png") # macc_byTech/sectors

```


```{r ag_lower}
macc_byTech_ag_lower = function(df, 
                       sectorSelect=NULL, 
                       sourceSelect=NULL, 
                       yearSelect=2030, 
                       priceMax=100) {
...
macc_byTech_ag_lower(MACC_df_ag, sectorSelect = "AGRICULTURE", yearSelect = 2030)
ggsave("output/AGRICULTURE_2030_v2024_lower.png") # macc_byTech/sectors
```


```{r sources_additional}

MACC_df_ag <- MACC_df
sourceSelect = "CROP"

macc_byTech_source_upper = function(df, 
                       #sectorSelect=NULL, 
                       sourceSelect=NULL, 
                       yearSelect=2030, 
                       priceMax=100) {
...
#  if(length(sectorSelect)>1) {rlang::abort(message = "This function can only handle one sector at a time")}
  ...
  df_summarized = df_filtered %>%
    group_by(year, p, tech, tech_long) %>% 
    summarize(q_total = sum(q_total), .groups = "drop")
...
macc_byTech_source_upper(MACC_df, sourceSelect = "CROP", yearSelect = 2030)


macc_byTech_source_lower = function(df, 
                       sectorSelect=NULL, 
                       sourceSelect=NULL, 
                       yearSelect=2030, 
                       priceMax=0) {
  ...
  #  macc_prepped_lower = macc_prepped %>%
  #    dplyr::filter(ymax == 0) %>%
  #    dplyr::filter(ymin == -100)
  #    #mutate(ymax = 0)
  #  title =  c(sectorSelect, ": Mitigation At or Below $0")
  # #}
   

fig2 <- macc_prepped %>% # _lower
  ggplot() +
  geom_rect(aes(xmin = xmin, ymin = ymin, xmax = xmax, ymax = ymax, fill = tech_long)) +
  #facet_grid(year~.) +
  #xlim(0, 500) +
...
macc_byTech_source_lower(MACC_df, sourceSelect = "LIVE", yearSelect = 2030)

```


```{r}
sectorSelect = "AGRICULTURE"
sourceSelect = "CROP"
yearSelect = c(2050)
 priceMax = 100
# region = c("North Carolina")
...
df_filtered = MACC_df %>%
 ...
 # }
  ...
  # check using geom_line on manually calculated cumsum  
  cumsum_df <- macc_prepped %>%
    arrange(p,.by_group = TRUE) %>%
    mutate(cumulative_q_total = cumsum(q_total)) 
  
  cumsum_df %>%
    ggplot() +
    geom_line(aes(x = cumulative_q_total, y = p)) +
    ylim(0, 100)
    
  
macc_prepped %>%
  ggplot() +
  geom_rect(aes(xmin = xmin, ymin = 0, xmax = xmax, ymax = ymax)) + #, fill = tech_long)) +
  #facet_grid(year~.) +
  #scale_fill_discrete(guide = guide_legend()) +
  theme(legend.position="none") +
  labs(y = "Price ($/tCO2e",
...
macc_smooth(df, sourceSelect = "CROP")
...
```{r sector_tables}
  MACC_mitigation_price_table_new <- macc_2019 %>%
...
write.csv(MACC_mitigation_price_table_new , "output/MACC_mitigation_price_table_new_2019.csv")
...
sector_total_mit <- MACC_df %>%
  group_by(sector, year) %>%
  summarise(total_mit = sum(q_total))

source_total_mit <- MACC_df %>%
  group_by(sector, source, year) %>%
  summarise(total_mit = sum(q_total))

  MACC_mitigation_price_table_sectors_new <- macc_2019 %>% #MACC_df
  #dplyr::filter(sector == "WASTE") %>%
    #mutate(price_group = case_when(p <= 0 ~ "at or below zero",
    #                               p >0 ~ "above zero")) %>%
  group_by(sector, year) %>%
...
write.csv(MACC_mitigation_price_table_sectors_new , "output/MACC_mitigation_price_table_sectors_new_2019.csv")
